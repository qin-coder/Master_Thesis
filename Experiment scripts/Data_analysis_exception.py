...
def process_files(file1, file2):
    ...
    for cls in classes:
        X = df1[df1['TARGET_CLASS'] == cls]['Implicit_MethodExceptions'].values
        Y = df2[df2['TARGET_CLASS'] == cls]['Implicit_MethodExceptions'].values
        ...
        if len(X) > 0 and len(Y) > 0:
            ...
            mean_X = np.mean(X) if len(X) > 0 else np.nan
            mean_Y = np.mean(Y) if len(Y) > 0 else np.nan
            std_X = np.std(X) if len(X) > 0 else np.nan
            std_Y = np.std(Y) if len(Y) > 0 else np.nan
...
def main():
    ...
    df_results = pd.DataFrame(results, columns=['Class', 'Â12', 'p-value', 'Default_Mean', 'RL_Mean', 'Dynamosa Std', 'RL-Dynamosa Std'])


    df_results['Default_Mean'] = df_results['Default_Mean'].map(lambda x: f"{x:.3f}" if not pd.isnull(x) else x)
    df_results['RL_Mean'] = df_results['RL_Mean'].map(lambda x: f"{x:.3f}" if not pd.isnull(x) else x)
    ...
    df_results.to_csv('Data_exception.csv', index=False)
