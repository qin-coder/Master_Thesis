...
//
        ...
        //RL_Listener rlListener = new RL_Listener();
        //algorithm.addListener(rlListener);
        LoggingUtils.getEvoLogger().info("Success to run Listener");
        //LoggingUtils.getEvoLogger().info("name of Listener" + rlListener);
...
//			ga.addListener(progressMonitor); // FIXME progressMonitor may cause
        ...
        if (!(Properties.STOP_ZERO && fitnessFunctions.isEmpty()) || ArrayUtil.contains(Properties.CRITERION, Criterion.EXCEPTION)) {
            ...
            LoggingUtils.getEvoLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Starting...");
